...
const STEPS = ["Income", "Debts", "Down Payment", "Credit", "Location"];
...
export default function Calculator() {
  ...
  const canNext = () => {
    switch (step) {
      case 0:
        return form.annual_income >= 10000;
      case 1:
        return form.monthly_debt >= 0;
      case 2:
        return form.down_payment >= 0;
      case 3:
        return !!form.credit_score;
      case 4:
        return !!form.state;
      default:
        return false;
  ...
  const submit = async () => {
    setSubmitting(true);
    try {
      // Generate estimate locally (no backend call)
    ...
    <section id="calculator" className="section bg-white">
      <div className="container-x">
        <div className="text-center max-w-2xl mx-auto mb-12">
          <span
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-[#0066FF] text-xs font-semibold uppercase tracking-wider"
            data-testid="calc-section-eyebrow"
          >
            <Sparkles className="w-3 h-3" /> Affordability Calculator
          </span>
          <h2
            className="font-heading text-4xl md:text-5xl font-semibold text-[#0F2557] mt-4 tracking-tight"
            data-testid="calc-section-title"
          >
            See your real budget in 60 seconds.
          </h2>
          <p className="mt-4 text-slate-600 text-lg">
            Five quick questions. We'll calculate what you can afford and
              ...
              <>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    {step === 0 && (
                      <div>
                        <Label className="font-heading text-2xl text-[#0F2557]">
                          What's your annual household income?
                        </Label>
                        <p className="text-sm text-slate-500 mt-1 mb-6">
                          Pre-tax income from all earners.
                        </p>
                        <div className="mb-2 flex items-baseline gap-2">
                          <span className="font-heading text-4xl font-bold text-[#0F2557] tabular-nums">
                            {formatMoney(form.annual_income)}
                          </span>
                          <span className="text-slate-500 text-sm">/ year</span>
                        </div>
                        <Slider
                          data-testid="calc-income-slider"
                          value={[form.annual_income]}
                          min={20000}
                          max={500000}
                          step={5000}
                          onValueChange={(v) =>
                            update("annual_income", v[0])
                          }
                        />
                    ...
                    {step === 1 && (
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1 mb-6">
                          Car loans, student loans, credit cards (minimum
                          payments).
                        </p>
                        <div className="mb-2 flex items-baseline gap-2">
                          <span className="font-heading text-4xl font-bold text-[#0F2557] tabular-nums">
                            {formatMoney(form.monthly_debt)}
                          </span>
                        ...
                        <Slider
                          data-testid="calc-debt-slider"
                          value={[form.monthly_debt]}
                          min={0}
                          max={5000}
                          step={50}
                          onValueChange={(v) => update("monthly_debt", v[0])}
                          className="mt-4"
                        />
                        <div className="flex justify-between text-xs text-slate-400 mt-2">
                          <span>$0</span>
                    ...
                    {step === 2 && (
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1 mb-6">
                          Including savings, gifts, or assistance programs.
                        </p>
                        <div className="mb-2 flex items-baseline gap-2">
                          <span className="font-heading text-4xl font-bold text-[#0F2557] tabular-nums">
                            {formatMoney(form.down_payment)}
                          </span>
                        </div>
                        <Slider
                          data-testid="calc-down-slider"
                          value={[form.down_payment]}
                          min={0}
                          max={200000}
                          step={1000}
                          onValueChange={(v) =>
                            update("down_payment", v[0])
                          }
                          className="mt-4"
                        />
                        <div className="flex justify-between text-xs text-slate-400 mt-2">
                    ...
                    {step === 3 && (
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1 mb-6">
                          A soft estimate is fine — no credit check.
                        </p>
                        <div className="grid sm:grid-cols-2 gap-3">
                          {CREDIT_OPTIONS.map((opt) => {
                            ...
                            return (
                              ...
                              >
                                <p className="font-heading font-semibold text-[#0F2557]">
                                  {opt.label}
                                </p>
                    ...
                    {step === 4 && (
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1 mb-6">
                          We'll match you with lenders licensed in your area.
                        </p>
                        <Select
                          value={form.state}
          ...
          <div className="lg:col-span-2">
            <div className="bg-[#0F2557] text-white rounded-2xl p-8 h-full relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <h3 className="font-heading text-3xl md:text-4xl font-semibold mt-2">
                  {result ? "Estimate" : "Estimate awaits"}
                </h3>
                {!result && (
                  <div className="mt-6 space-y-4">
                    <div className="flex items-start gap-3 text-blue-100">
                      ...
                      <p className="text-sm">
                        Home price range tailored to your income & DTI
                      </p>
                    </div>
                    <div className="flex items-start gap-3 text-blue-100">
                      <CheckCircle2 className="w-5 h-5 mt-0.5 text-blue-300 flex-shrink-0" />
                      <p className="text-sm">
                        Estimated monthly payment & today's rates
                      </p>
                    </div>
                    <div className="flex items-start gap-3 text-blue-100">
                      <CheckCircle2 className="w-5 h-5 mt-0.5 text-blue-300 flex-shrink-0" />
                      <p className="text-sm">
                        Top 3 loan programs you likely qualify for
                      </p>
                    </div>
                    <div className="flex items-start gap-3 text-blue-100">
                      <CheckCircle2 className="w-5 h-5 mt-0.5 text-blue-300 flex-shrink-0" />
                      <p className="text-sm">
                        No credit check · No spam · Free
                      </p>
                    </div>
                  </div>
                )}
                {result && (
                  <div className="mt-6 space-y-3">
                    <div className="flex items-center gap-2 text-blue-100 text-sm">
                      <TrendingUp className="w-4 h-4" /> Based on a 30-year
                      fixed at {result.estimated_rate}%
                    </div>
                  </div>
                )}
...
function ResultView({ result, form, onReset }) {
  ...
  return (
    <div data-testid="calc-result-view">
      ...
      <h3
        className="font-heading text-3xl md:text-4xl font-semibold text-[#0F2557]"
        data-testid="calc-result-title"
      >
        Here's your personalized budget.
      </h3>
      <p className="text-slate-600 mt-2">
        A licensed loan officer will follow up shortly. No obligation.
      </p>
      ...
      <div className="mt-6 grid sm:grid-cols-2 gap-4">
        <div className="bg-blue-50/60 rounded-xl p-5 border border-blue-100">
          ...
          <p
            className="font-heading text-3xl font-bold text-[#0F2557] mt-2 tabular-nums"
            data-testid="result-home-price"
          >
            {formatMoney(result.home_price_low)} –{" "}
            {formatMoney(result.home_price_high)}
          </p>
        </div>
        <div className="bg-blue-50/60 rounded-xl p-5 border border-blue-100">
          <p className="text-xs font-semibold uppercase tracking-wider text-[#0066FF]">
      ...
      <div className="mt-6">
        ...
        <div className="flex flex-wrap gap-2">
          {result.loan_programs.map((p) => (
            <span
              key={p}
              className="px-3 py-1.5 bg-white border border-gray-200 rounded-full text-sm font-medium text-[#0F2557]"
              data-testid="result-program"
            >
              {p}
            </span>
          ))}
      ...
      <div className="mt-6 grid grid-cols-3 gap-3 text-center">
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <p className="font-semibold text-[#0F2557] tabular-nums">
      ...
      <div className="mt-12 border-t pt-10">
        ...
        <iframe
          ...
          style={{ width: "100%", height: "2000px", border: "none", borderRadius: "8px" }}
        />
      </div>
    </div>
  );
